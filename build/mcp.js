...
class MCPClient {
    ...
    toolRegistry = new Map(); // toolName -> serverId
    serverInstructions = new Map(); // serverId -> instructions
    tools = [];
    ollama;
    model;
    ...
    async connectToStdioServer(client, serverId, command, args) {
        try {
    ...
    async connectToHttpServer(client, serverId, url) {
        try {
            // Try StreamableHTTPClientTransport first (modern and recommended)
            const transport = new StreamableHTTPClientTransport(new URL(url));
            await client.connect(transport);
            console.error(`Successfully connected to HTTP server ${serverId} via StreamableHTTPClientTransport`);
        }
        catch (e) {
            console.error(`Failed to connect via StreamableHTTPClientTransport for ${serverId}, falling back to SSEClientTransport`);
            try {
                // Fallback to SSEClientTransport for servers that still use SSE
                const transport = new SSEClientTransport(new URL(url));
                await client.connect(transport);
                console.error(`Successfully connected to HTTP server ${serverId} via SSEClientTransport`);
            }
            catch (fallbackError) {
                console.error(`Failed to connect to HTTP MCP server ${serverId} with both transports:`, fallbackError);
                throw fallbackError;
            }
        }
    }
    async listTools(client, serverId) {
        try {
            ...
            console.error(`  Tools from ${serverId}: ${serverTools.map((t) => t.name).join(", ")}`);
        }
        catch (e) {
            console.warn(`Could not list tools from server ${serverId}:`, e);
        }
    }
    async listResources(client, serverId) {
        try {
            const resourcesResult = await client.listResources();
            const resources = resourcesResult.resources || [];
            // Look for instructions resource
            for (const resource of resources) {
                if (resource.name.includes("instruction") ||
                    ...
                    try {
                        ...
                        console.warn(`Could not read resource ${resource.uri} from server ${serverId}:`, e);
                    }
                }
            }
        }
            ...
            if (e?.code !== -32601) {
                console.warn(`Could not list resources from server ${serverId}:`, e.message || e);
            }
    ...
    async callTool(toolName, toolArguments) {
        ...
        if (!client) {
            throw new Error(`Server ${serverId} (providing tool ${toolName}) is not connected`);
        }
        try {
            ...
            throw new Error(`Error calling tool ${toolName} on server ${serverId}: ${e}`);
        }
    }
    async cleanup() {
